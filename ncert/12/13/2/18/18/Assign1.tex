...
\raggedright\textbf{Problem Statement:}\\{Two events A and B will be independent, if}\\
\begin{enumerate}
\item  A and B are mutually exclusive\\
\item  $\pr{A^\prime B^\prime}=[1-\pr{A}] [1-\pr{B}]$\\
\item  $\pr{A} = \pr{B}$\\
\item  $\pr{A} + \pr{B} = 1$\\ 
\end{enumerate}
...
\begin{align}
 ...
 Hence it shows A and B are Independent events \\
  \item For the same counter example given for option A, \pr{A} = \pr{B}, But A and B are not independent events.\\
  \item  For the same counter example given for option A, \pr{A} + \pr{B} = 1, But A and B are not independent events.
  \end{enumerate}
 \end{document}
